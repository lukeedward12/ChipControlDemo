...
uint8_t ChipControl_Set_ADC_Mode(SetADCModeInput *input) {
  ...
  if (input->mode != SLEEP || input->mode != MANUAL ||
      input->mode != SCAN || input->mode != AUTO) {
  ...
  // Combine current Control Register (Bits [0:5]) with Desired ADC Mode
  // (Bits [6:7])
...
uint8_t ChipControl_Get_Temp_Status(GetTempStatusResponse *response) {
  ...
  // To get the temp alert mask, AND the probed Status Register with the
...
uint8_t ChipControl_Set_Charge_Thresholds(SetChargeThresholdInput *input) {
  ...
  // First, determine if Max or Min desired Charge Threshold is within
  // battery limits of 0 mAh <-> 2228 mAh
  ...
  // Convert input threshold (mAh) to 16 bit fixed width such that 0.034
  ...
  uint8_t min_threshold_fixed_LSB = min_threshold_fixed & LSB_16BIT_MASK;
